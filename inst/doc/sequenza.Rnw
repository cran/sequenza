...
Since many projects might already being processed trough VarScan2, it can be convenient to be able to import such results.
...
The result is a list in the format \textit{list(x ,y, z)} directly usable by standard graphical function, like \textit{image}. However we provide few customized functions to explore and better display the results, detect the confidence intervals, and finally retrieve the point estimate.

\subsection{Results of model fitting}

The last part of the workflow, would be to apply the estimated parameters. There is a all in one function that would plot and save the results, giving control on file names and output  directory:
...
However this standard way of present the result would satisfy the usual situation, it is possible to create alternative wrapper by using functions on the following sub-sections.
